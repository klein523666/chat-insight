...
const DEFAULT_REPORT_PROMPT = "根据本次报告窗口的原始消息和数据概览，自行调整分析重点：先识别消息量、来源集中度、关键词与讨论变化，再优先呈现具有行动价值、风险或明确需求的信号。";
...
function Tasks({ sources, targets, tasks, reload }: { sources:Source[];targets:Target[];tasks:Task[];reload:()=>void }) {
  ...
  return <><header className="compact-head"><span className="kicker">REPORT AUTOMATION</span><h1>报告任务</h1><p>来源、自然时间窗口、连续迭代提示词和投递目标被绑定为一个可审计任务。</p></header><section className="task-layout"><form key={editing?.id??"new"} className="paper task-form" onSubmit={save}><div className="form-heading"><b>{editing?"编辑任务":"新建任务"}</b>{editing&&<button type="button" className="text-button" onClick={cancelEdit}>取消编辑</button>}</div><Field label="任务名称"><input name="name" placeholder="全平台 AI 日报" defaultValue={editing?.name} required/></Field><label className="task-enabled"><input name="enabled" type="checkbox" defaultChecked={editing?.enabled??true}/><span>启用此任务</span></label><div className="choice-block"><b>01 / 选择来源</b>{sources.filter(x=>x.enabled).map(x=><label key={x.id}><input type="checkbox" name="sources" value={x.id} defaultChecked={editing?.source_ids.includes(x.id)}/><span>{x.title}</span><small>{x.platform}</small></label>)}</div><div className="form-row"><Field label="周期"><select name="schedule_type" defaultValue={editing?.schedule_type??"daily"}><option value="daily">日报</option><option value="hourly">小时报</option></select></Field><Field label="小时"><input name="hour" type="number" min="0" max="23" defaultValue={editing?.schedule_hour??23}/></Field><Field label="分钟"><input name="minute" type="number" min="0" max="59" defaultValue={editing?.schedule_minute??55}/></Field></div><Field label="时区"><input name="timezone" defaultValue={editing?.timezone??"Asia/Shanghai"} required/></Field><fieldset className="prompt-control"><legend>02 / 当前迭代提示词</legend><p>本次报告先使用此提示词；自动迭代会在报告成功后，依据本次原始消息更新为下一轮提示词。聊天消息始终作为不可信数据处理。</p><label><input type="radio" name="prompt_mode" value="adaptive" checked={promptMode==="adaptive"} onChange={()=>setPromptMode("adaptive")}/><span><b>自动迭代</b><small>每次报告后保留你的目标，并更新下一轮的关注重点。</small></span></label><label><input type="radio" name="prompt_mode" value="custom" checked={promptMode==="custom"} onChange={()=>setPromptMode("custom")}/><span><b>手动固定</b><small>使用下方提示词生成报告，不自动修改。</small></span></label><textarea name="report_prompt" defaultValue={editing?.report_prompt??DEFAULT_REPORT_PROMPT} maxLength={4000} placeholder="例如：优先总结产品反馈和购买意向；按紧急程度列出待跟进事项。" required aria-label="当前迭代提示词"/></fieldset><div className="choice-block"><b>03 / 飞书目标</b>{targets.map(x=><label key={x.id}><input type="checkbox" name="targets" value={x.id} defaultChecked={editing?.delivery_target_ids.includes(x.id)}/><span>{x.name}</span></label>)}</div><button className="primary" disabled={saving}>{saving?"正在保存…":submitLabel}</button>{message&&<p className="success">{message}</p>}{error&&<p className="error">{error}</p>}</form><div className="task-stack">{tasks.length?tasks.map((x,i)=><article key={x.id}><span className="task-no">{String(i+1).padStart(2,"0")}</span><div><b>{x.name}</b><p>{x.schedule_type==="daily"?`每日 ${String(x.schedule_hour).padStart(2,"0")}:${String(x.schedule_minute).padStart(2,"0")}`:`每小时 :${String(x.schedule_minute).padStart(2,"0")}`} · {x.timezone}</p><small className="task-prompt">{x.prompt_mode==="custom"?"手动固定":"自动迭代"}</small><p className="task-prompt-preview" title={x.report_prompt}>提示词 · {x.report_prompt}</p></div><div className="task-state"><StatusDot value={x.enabled?"online":"disabled"}/><button type="button" className="secondary" onClick={()=>beginEdit(x)} aria-label={`编辑报告任务 ${x.name}`}>编辑</button><button type="button" className="secondary danger-button" onClick={()=>void removeTask(x)} disabled={saving} aria-label={`删除报告任务 ${x.name}`}>删除</button></div></article>):<Empty>选择至少一个已启用来源来创建任务。</Empty>}</div></section></>;
}
...
export default function App() {
  ...
  return <div className="app"><aside><div className="logo"><i>CI</i><span>CHAT<br/><b>INSIGHT</b></span></div><nav>{NAV.map(x=><button key={x.id} className={page===x.id?"active":""} onClick={()=>setPage(x.id)}><small>{x.mark}</small>{x.label}</button>)}</nav><footer><span className="pulse"/>LOCAL ONLY<small>v0.1.2</small></footer></aside><main className="workspace">{error&&<div className="toast">{error}<button onClick={()=>setError("")}>×</button></div>}{content[page]}</main></div>;
}
