...
type Task = { id: number; name: string; enabled: boolean; schedule_type: string; schedule_hour: number; schedule_minute: number; timezone: string; source_ids: number[]; delivery_target_ids: number[] };
...
function Sources({ values, reload }: { values: Source[]; reload: () => void }) {
  const [filter,setFilter]=useState("all"); const [folder,setFolder]=useState("all"); const folders=[...new Set(values.filter(x=>x.platform==="telegram").flatMap(x=>x.folders))].sort(); const shown=values.filter(x=>(filter==="all"||x.platform===filter)&&(folder==="all"||x.folders.includes(folder)));
  async function toggle(item:Source){ await mutate(`/api/v1/sources/${item.id}`,"PATCH",{enabled:!item.enabled}); reload(); }
  return <><header className="compact-head"><span className="kicker">SOURCE CONSENT</span><h1>消息源</h1><p>发现不等于采集。只有开启的来源才会持久化并进入报告。</p></header><div className="toolbar"><button className={filter==="all"?"selected":""} onClick={()=>{setFilter("all");setFolder("all")}}>全部 {values.length}</button><button className={filter==="qq"?"selected":""} onClick={()=>{setFilter("qq");setFolder("all")}}>QQ</button><button className={filter==="telegram"?"selected":""} onClick={()=>setFilter("telegram")}>Telegram</button></div>{(filter==="telegram"||filter==="all")&&folders.length>0&&<div className="toolbar" style={{alignItems:"center",flexWrap:"wrap",marginTop:-8}}><span style={{font:"800 10px monospace",letterSpacing:".12em",color:"var(--muted)",paddingRight:8}}>Telegram 分组</span><button className={folder==="all"?"selected":""} onClick={()=>setFolder("all")}>全部</button>{folders.map(name=><button className={folder===name?"selected":""} key={name} onClick={()=>{setFilter("telegram");setFolder(name)}}>{name}</button>)}</div>}<section className="source-list">{shown.length ? shown.map(item=><article key={item.id}><div className="source-mark">{item.platform==="qq"?"Q":"T"}</div><div><b>{item.title||item.id}</b><span>{item.platform} · {item.chat_type}{item.folders.length?` · ${item.folders.join(" / ")}`:""}</span></div><StatusDot value={item.status}/><label className="switch"><input type="checkbox" checked={item.enabled&&item.status!=="migrated"} disabled={item.status==="migrated"} onChange={()=>toggle(item)}/><i/><span>{item.status==="migrated"?"已迁移":item.enabled?"采集中":"已关闭"}</span></label></article>) : <Empty>Collector 登录后会自动发现可用群与频道。</Empty>}</section></>;
}
...
function AIAndDelivery({ targets, reload }: { targets: Target[]; reload:()=>void }) {
  ...
  return <><header className="compact-head"><span className="kicker">ANALYSIS & DELIVERY</span><h1>AI 与飞书</h1><p>模型只收到被选中的文本，不具备工具，也不会接触内部用户 ID。</p></header><section className="settings-grid"><form className="paper form-card" onSubmit={saveAI} autoComplete="off"><div className="section-title"><span>AI</span><h2>OpenAI Compatible</h2></div><Field label="BASE URL"><input name="ai_base_url" defaultValue="https://api.openai.com/v1" autoComplete="off" spellCheck={false} required /></Field><Field label="MODEL" hint="可从预设中选择，也可输入兼容服务的模型 ID"><input name="ai_model" list="model-presets" defaultValue="gpt-5.4-mini" autoComplete="off" spellCheck={false} required /><datalist id="model-presets">{MODEL_PRESETS.map(([value,label])=><option key={value} value={value} label={label}/>)}</datalist></Field><Field label="API KEY" hint="留空不会覆盖已有密钥"><input name="ai_access_token" type="password" autoComplete="section-ai new-password" /></Field><button className="primary">保存 AI 配置</button>{message&&<p className="success">{message}</p>}</form>
  <form className="ink-card form-card" onSubmit={addTarget} autoComplete="off"><div className="section-title"><span>飞</span><h2>飞书目标</h2></div><Field label="名称"><input name="feishu_target_name" placeholder="运营群" autoComplete="off" required /></Field><Field label="WEBHOOK"><input name="feishu_webhook_url" type="url" autoComplete="off" spellCheck={false} required /></Field><Field label="SIGN SECRET"><input name="feishu_signing_key" type="password" autoComplete="section-feishu new-password" /></Field><button>新增推送目标</button><div className="chips">{targets.map(x=><span key={x.id}>{x.name}</span>)}</div></form></section></>;
}

function Tasks({ sources, targets, tasks, reload }: { sources:Source[];targets:Target[];tasks:Task[];reload:()=>void }) {
  async function create(e:FormEvent<HTMLFormElement>){e.preventDefault();const form=new FormData(e.currentTarget);await mutate("/api/v1/report-tasks","POST",{name:form.get("name"),source_ids:form.getAll("sources").map(Number),schedule_type:form.get("schedule_type"),schedule_hour:Number(form.get("hour")),schedule_minute:Number(form.get("minute")),timezone:form.get("timezone"),delivery_target_ids:form.getAll("targets").map(Number)});e.currentTarget.reset();reload();}
  return <><header className="compact-head"><span className="kicker">REPORT AUTOMATION</span><h1>报告任务</h1><p>来源、自然时间窗口和投递目标被绑定为一个可审计任务。</p></header><section className="task-layout"><form className="paper task-form" onSubmit={create}><Field label="任务名称"><input name="name" placeholder="全平台 AI 日报" required/></Field><div className="choice-block"><b>01 / 选择来源</b>{sources.filter(x=>x.enabled).map(x=><label key={x.id}><input type="checkbox" name="sources" value={x.id}/><span>{x.title}</span><small>{x.platform}</small></label>)}</div><div className="form-row"><Field label="周期"><select name="schedule_type"><option value="daily">日报</option><option value="hourly">小时报</option></select></Field><Field label="小时"><input name="hour" type="number" min="0" max="23" defaultValue="23"/></Field><Field label="分钟"><input name="minute" type="number" min="0" max="59" defaultValue="55"/></Field></div><Field label="时区"><input name="timezone" defaultValue="Asia/Shanghai" required/></Field><div className="choice-block"><b>02 / 飞书目标</b>{targets.map(x=><label key={x.id}><input type="checkbox" name="targets" value={x.id}/><span>{x.name}</span></label>)}</div><button className="primary">创建任务 →</button></form><div className="task-stack">{tasks.length?tasks.map((x,i)=><article key={x.id}><span className="task-no">{String(i+1).padStart(2,"0")}</span><div><b>{x.name}</b><p>{x.schedule_type==="daily"?`每日 ${String(x.schedule_hour).padStart(2,"0")}:${String(x.schedule_minute).padStart(2,"0")}`:`每小时 :${String(x.schedule_minute).padStart(2,"0")}`} · {x.timezone}</p></div><StatusDot value={x.enabled?"online":"disabled"}/></article>):<Empty>选择至少一个已启用来源来创建任务。</Empty>}</div></section></>;
}
...
export default function App() {
  ...
  return <div className="app"><aside><div className="logo"><i>CI</i><span>CHAT<br/><b>INSIGHT</b></span></div><nav>{NAV.map(x=><button key={x.id} className={page===x.id?"active":""} onClick={()=>setPage(x.id)}><small>{x.mark}</small>{x.label}</button>)}</nav><footer><span className="pulse"/>LOCAL ONLY<small>v0.1.0</small></footer></aside><main className="workspace">{error&&<div className="toast">{error}<button onClick={()=>setError("")}>×</button></div>}{content[page]}</main></div>;
}
